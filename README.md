...
VoiceCal Agent 是一个语音驱动的 AI 日历助手 Demo 项目，支持用户通过语音或文本与日历交互，并通过 Spring Boot 后端提供日程管理、冲突检测、空闲时间查询、每日摘要、提醒状态、操作日志等能力。
...
- ICS 导出入口：前端事件详情中提供 ICS 下载入口；请以当前后端分支实际 Controller 为准确认导出接口是否已落地。
