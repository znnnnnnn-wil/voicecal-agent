...
VoiceCal Agent 是一个语音驱动的 AI 日历助手。用户可以通过语音或文本输入自然语言指令，完成日程创建、查询、修改、删除、冲突检测、空闲时间查询、每日摘要和日历可视化展示。
...
- 如果未配置真实模型，接口会返回 fallback 文案，前端仍能演示语音输入、文本兜底、语音播报和降级状态。
...
1. AI 识别删除意图和目标日程。
2. 如果目标明确，后端 Calendar Tools 直接执行删除。
3. 如果目标不明确或存在多个候选，AI 应先让用户补充信息。
4. 日历、今日安排、本周摘要和操作日志会在操作完成后刷新。
...
- 后端还支持按时间范围导出：`GET /api/calendar/events/ics?startTime=...&endTime=...`。
